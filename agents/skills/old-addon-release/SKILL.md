...
- Type: `single_select`
- Options: ["Tag release", "Do nothing"]

Handle selections as follows:

- **"Tag release"** → determine the release tag from the changelog (e.g. `v1.2.3`),
  then run the following via `bash_tool`:
```
  git add CHANGELOG.md && git commit -m "changelog" && git push && gtag [release]
```
  Confirm to the user once complete, showing the tag that was pushed.

- **"Do nothing"** → acknowledge and end the workflow.
